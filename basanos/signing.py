...
class ProviderSigner:
    ...
    def _read_existing_seed(self) -> bytes:
        ...
        with os.fdopen(descriptor, "rb") as handle:
            ...
            if not stat.S_ISREG(opened_info.st_mode) or (opened_info.st_dev, opened_info.st_ino) != (
                path_info.st_dev,
                path_info.st_ino,
            ):
    ...
    def seed(self) -> bytes:
        ...
        Handed to ``oracle_core.Signer`` in memory (see basanos.federation) so the federation
        manifest is signed by the SAME identity as the assurance pack. Two keypairs would
        make the manifest advertise one public key while capability.json and the pack
        signature advertise another — a verifier following the chain would stop there.
        """
        return self.private.private_bytes_raw()

    def sign_canonical(self, canonical: str) -> str:
        """Sign an already-canonicalised string. The caller owns the canonical form."""
    ...
    def sign_result(
        ...
        return self.sign_canonical(canonical)
